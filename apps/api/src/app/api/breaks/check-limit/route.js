...
export async function POST(req) {
  try {
    ...
    if (!breakRecord) {
      return Response.json({
        success: true,
        message: "Break not found, ignoring",
      });
    }
    ...
    if (breakRecord.endedAt) {
      ...
      return Response.json({
        success: true,
        message: "Break ended early, no alert sent",
      });
    }
    ...
    if (user && user.pushSubscription) {
      ...
      let bodyText =
        "Your break ends in 1 minute! Please stop your break and return to work ASAP.";

      if (triggerType === "9min") {
        titleText = "9-Minute Break Warning";
        bodyText =
          "You have been on break for 9 minutes. Reaching 15 minutes will count as 2 breaks! Please return soon.";
      } else if (triggerType === "14min") {
        titleText = "14-Minute Break Warning";
        bodyText =
          "You have been on break for 14 minutes. If you exceed 15 minutes, it will count as 2 breaks! Return to work now.";
        ...
        if (pushErr.statusCode === 410) {
          try {
            await db
              ...
              .where(eq(users.id, userId));
            console.log(
              `Cleaned up stale push subscription for user ${userId}`,
            );
          } catch (cleanupErr) {
      ...
      try {
        ...
        for (const n of notifiers) {
          ...
          try {
            ...
            console.error(
              "Failed to notify HR/Admin about break limit:",
              pushErr,
            );
            if (pushErr.statusCode === 410) {
              try {
                await db
                  .update(users)
                  .set({ pushSubscription: null })
                  .where(eq(users.id, n.id));
              } catch (cleanupErr) {
                console.error(
                  "Failed to clean up HR/Admin push subscription:",
                  cleanupErr,
                );
              }
            }
        ...
        console.error(
          "Failed to fetch HR/Admin for break notification:",
          dbErr,
        );
      }
    }
    ...
    return Response.json({
      success: true,
      message: "Over-limit alert processed",
    });
  } catch (error) {
