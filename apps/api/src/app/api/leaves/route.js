...
async function postHandler(req, { ctx }) {
  try {
    const { startDate, endDate, type, reason, isHalfDay, halfDayPeriod } =
      await req.json();
    ...
    if (!isHalfDay) {
      const diffDays = Math.ceil(
        (startNormalized.getTime() - today.getTime()) / (1000 * 60 * 60 * 24),
      );
      if (diffDays < 2) {
        return Response.json(
          {
      ...
      return Response.json(
        {
          success: false,
          message:
            "You already have a leave application that overlaps with these dates.",
        },
        { status: 400 },
      );
    }
          ...
          return Response.json(
            {
              success: false,
              message:
                "Morning half-days must be applied before 9:30 AM on the same day.",
            },
            { status: 400 },
          );
        }
          ...
          return Response.json(
            {
              success: false,
              message:
                "Evening half-days must be applied before 2:00 PM on the same day.",
            },
            { status: 400 },
          );
        }
    ...
    if (blocks.length === 0) {
      return Response.json(
        {
          success: false,
          message: "Selected leave period only contains Sundays (off days).",
        },
        { status: 400 },
      );
    }
        ...
        return Response.json(
          {
            success: false,
            message:
              "You have already used your paid leave allowance for this month.",
          },
          { status: 400 },
        );
      }
      ...
      if (requestedDays > maxAllowedDays) {
        if (maxAllowedDays === 1.0 && !isHalfDay && requestedDays > 1) {
          ...
          return Response.json(
            {
              success: false,
              message: `Leave limit exceeded. You have ${maxAllowedDays} paid leaves remaining this month.`,
            },
            { status: 400 },
          );
        }
    ...
    try {
      ...
      for (const n of notifiers) {
        sendLeaveNotification({
          ...
          ctx.error("LEAVE_NOTIFICATION_FAILED", {
            userId: n.name,
            error: err.message,
          }),
        );
