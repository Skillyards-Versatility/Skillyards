...
async function postHandler(req, { ctx }) {
  ...
  const expectedSecret =
    process.env.CALL_TRACKER_SECRET || "skillyards_call_tracker_secret_default";
  ...
  const EMPLOYEE_TO_USER_MAP = {
    "f2b05e4f-e983-4c35-abb3-600f5a730a45":
      "0f164502-26f3-4d9a-adcc-703adfb1af33", // Rahul Singh
    "a22ac5ef-452b-46ed-9b77-43529200c08c":
      "c6a0af52-b579-4d64-9b72-7d64e5044840", // Suryansh Upadhyay
    "13c29d1d-f765-4483-9bcc-3aeeb14942f7":
      "70c7f340-888b-4380-a9b4-30123f1f8a33", // Karan Singh Tomar
    "988f616e-bea5-4c98-a8c1-b160a7aca469":
      "4d3e6da0-4f82-49db-9781-2273e52572ae", // Saurabh Verma
  };
  ...
  if (EMPLOYEE_TO_USER_MAP[telecaller_id]) {
    ...
    console.log(
      `[Mapping] Transformed old employee UUID ${telecaller_id} to new user UUID ${activeTelecallerId}`,
    );
  }
  ...
  try {
    ...
    if (userExists.length === 0) {
      ctx.warn("CALL_TRACKER_INVALID_TELECALLER", {
        telecaller_id: activeTelecallerId,
      });
      // Return 200 OK so the mobile client removes this call from its offline Room database queue
      return Response.json({
        success: true,
        message: "Call skipped (invalid or deactivated telecaller)",
      });
    }
  ...
  try {
    ...
    if (existing.length > 0) {
      ctx.log("CALL_RECORDING_LOGGED_DUPLICATE_SKIPPED", {
        telecaller_id: activeTelecallerId,
        cleanPhone,
        call_start_time,
      });
      return Response.json({
        success: true,
        message: "Call already logged (duplicate skipped)",
      });
    }
  } catch (dedupError) {
