 ...
 */
export const sendReceiptEmail = withResend(
  (resend, { to, studentName, receiptNumber, pdfBuffer }) => {
 ...
 */
export const sendEodReportEmail = withResend(
  async (resend, { to, bcc, team, date, reports, missingUsers, adminUrl }) => {
 ...
 */
export const sendEodWarningEmail = withResend(
  async (resend, { to, userName, date, adminUrl }) => {
    if (!(await isFeatureEnabled("eod_emails_feature"))) {
      console.warn("Skipping EOD email — eod_emails_feature is disabled");
      return;
    }
 ...
 */
export const sendEodAllTeamsEmail = withResend(
  async (resend, { to, date, teamSummaries, adminUrl }) => {
    if (!(await isFeatureEnabled("eod_emails_feature"))) {
      console.warn("Skipping EOD email — eod_emails_feature is disabled");
      return;
    }
 ...
 */
export const sendPasswordResetEmail = withResend(
  (resend, { to, resetLink }) => {
    const from = process.env.EMAIL_FROM || "Skillyards <admin@skillyards.in>";
 ...
 */
export const sendLeaveNotification = withResend(
  (resend, { to, recipientName, leave }) => {
    const from = process.env.EMAIL_FROM || "Skillyards <admin@skillyards.in>";
 ...
 */
export const sendLeaveStatusNotification = withResend(
  (
    resend,
    {
      to,
