...
const BlogSearch = ({ posts }) => {
  ...
  const paginated = debouncedQuery ? filtered : filtered.slice(0, visibleCount);
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      setFocusedIndex((prev) =>
        prev < paginated.length - 1 ? prev + 1 : prev,
      );
        ...
        <div className="space-y-12">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {paginated.map((post, index) => (
              <div
                key={post._id}
                className={`transition-all duration-300 rounded-3xl h-full ${
                  focusedIndex === index
                    ? "ring-4 ring-primary ring-offset-4 scale-[1.03] shadow-2xl z-10"
                    : ""
                }`}
              >
                <BlogCard
        ...
        <div className="flex flex-col items-center justify-center py-24 gap-8 border border-border/40 rounded-[2.5rem] bg-background/40 backdrop-blur-md shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(255,255,255,0.02)] text-center relative overflow-hidden">
          ...
          <div className="space-y-4 relative z-10">
            ...
            <p className="text-base text-muted-foreground max-w-sm mx-auto">
              Don&apos;t let the search end here. Try exploring one of our
              recommended topics:
            </p>
          </div>
