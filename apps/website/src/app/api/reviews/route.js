...
export async function GET() {
  ...
  if (!apiKey || !placeId) {
    console.warn("Google Places API Key or Place ID not configured. Returning fallback mock reviews.");
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Places API request failed with status: ${response.status}. Returning fallback reviews.`);
      return NextResponse.json(FALLBACK_DATA);
    }
    ...
    const result = {
      ...
      reviews: (data.reviews || []).map((review) => ({
        ...
        relativePublishTimeDescription: review.relativePublishTimeDescription || "Recent",
        text: {
