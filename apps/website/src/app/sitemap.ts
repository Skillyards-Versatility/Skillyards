...
import {
  getAllTeamSlugs,
  getAllTeamMembers,
} from "@/lib/sanity/getTeamMembers";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  for (const dir of PHOTO_DIRS) {
    staticPhotos[dir] = listPhotoFiles(join(photoDir, dir)).map((f) =>
      escapeXml(`${BASE_URL}/images/${dir}/${f}`),
    );
  }
  ...
  const findEntry = (urlPath: string) =>
    [...staticUrls, ...blogUrls, ...teamUrls].find(
      (e) => e.url === `${BASE_URL}${urlPath}`,
    );
