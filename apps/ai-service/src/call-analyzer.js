...
export async function auditCall(recordingKey, followUpId) {
  ...
  try {
    ...
    console.log(
      `[Call Analyzer] Audio size: ${sizeInMb.toFixed(2)} MB, extension: ${ext}`,
    );
    ...
    if (sizeInMb >= 10) {
      ...
      console.log(
        `[Call Analyzer] Uploaded to Gemini Files API: ${fileRef.name}`,
      );
      contents = [
        fileRef,
        {
    ...
    if (finishReason === "MAX_TOKENS") {
      console.warn(
        "[Call Analyzer] Warning: Response was truncated due to output token limit.",
      );
    }
    ...
    try {
      ...
      console.error(
        "[Call Analyzer] Error parsing Gemini JSON response. Raw text length:",
    ...
    console.log(
      `[Call Analyzer] Successfully stored audit results for followUpId: ${followUpId}`,
    );
    return auditData;
  } finally {
    ...
    if (tempFilePath && fs.existsSync(tempFilePath)) {
      try {
        ...
        console.log(
          `[Call Analyzer] Cleaned up temporary file: ${tempFilePath}`,
        );
      } catch (err) {
        console.error(
          `[Call Analyzer] Failed to clean up temp file: ${tempFilePath}`,
          err,
        );
      }
    }
    ...
    if (fileRef) {
      try {
        ...
        console.log(
          `[Call Analyzer] Deleted file from Gemini Files API: ${fileRef.name}`,
        );
      } catch (err) {
        console.error(
          `[Call Analyzer] Failed to delete Gemini file: ${fileRef.name}`,
          err,
        );
      }
    }
