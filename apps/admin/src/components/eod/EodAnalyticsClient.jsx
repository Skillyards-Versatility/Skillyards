...
export function EodAnalyticsClient({
  isAdmin = false,
  isManager = false,
  userName = null,
}) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getEodAnalytics({
        startDate,
        endDate,
  ...
  useEffect(() => {
    if (
      selectedUser &&
      userAvailableMetrics.length > 0 &&
      (!selectedUserMetric ||
        !userAvailableMetrics.includes(selectedUserMetric))
    ) {
  ...
  const renderDrillDownView = () => {
    if (userAvailableMetrics.length === 0) {
      return (
        <div className="flex flex-col items-center justify-center py-24 text-muted-foreground border border-dashed rounded-xl bg-slate-50/50 dark:bg-slate-900/20">
          <p className="text-base font-medium">
            No numerical data found for {selectedUser}.
          </p>
        </div>
      );
    }

    const totalValue = userTimeSeries.reduce(
      (sum, item) => sum + (item[selectedUserMetric] || 0),
      0,
    );
    ...
    if (isSalesRole) {
      const getDayTargetDialed = (day) => {
        const totalCouns =
          (day.counsellingDone || 0) + (day.walkinCounselling || 0);
      ...
      for (let i = userTimeSeries.length - 1; i >= 0; i--) {
        ...
        if (
          (d.dialedCalls || 0) >= getDayTargetDialed(d) &&
          (d.connectedCalls || 0) >= getDayTargetConnected(d) &&
          (d.talkTime || 0) >= TARGET_TALK_TIME
        ) {
          tempStreak++;
        } else {
          break;
      ...
      userTimeSeries.forEach((day) => {
        if (
          (day.dialedCalls || 0) >= getDayTargetDialed(day) &&
          (day.connectedCalls || 0) >= getDayTargetConnected(day) &&
          (day.talkTime || 0) >= TARGET_TALK_TIME
        ) {
          highestTemp++;
    ...
    const latestDay =
      userTimeSeries.length > 0
        ? userTimeSeries[userTimeSeries.length - 1]
        : null;
    ...
    const latestTotalCouns =
      (latestDay?.counsellingDone || 0) + (latestDay?.walkinCounselling || 0);
    const dayTargetDialed = Math.max(0, 120 - latestTotalCouns * 15);
    const dayTargetConnected = Math.max(30, 50 - latestTotalCouns * 5);

    const batCalls =
      dayTargetDialed > 0
        ? Math.min(100, Math.round((latestDialed / dayTargetDialed) * 100))
        : 100;
    const batConn =
      dayTargetConnected > 0
        ? Math.min(
            100,
            Math.round((latestConnected / dayTargetConnected) * 100),
          )
        : 100;
    const batTalk = Math.min(
      100,
      Math.round((latestTalkTime / TARGET_TALK_TIME) * 100),
    );
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        {isSalesRole && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in zoom-in-95 duration-500">
            <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-xs relative overflow-hidden flex flex-col items-center justify-center min-h-[220px]">
              ...
              <h3 className="font-semibold text-sm text-muted-foreground mb-4 uppercase tracking-wider w-full text-left">
                Daily Targets
              </h3>
              ...
              <div className="w-full space-y-4">
                <div>
                  <div className="flex justify-between text-xs font-medium mb-1">
                    ...
                    <span className={batCalls >= 100 ? "text-emerald-500" : ""}>
                      {latestDialed}
                    </span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs font-medium mb-1">
                    ...
                    <span className={batConn >= 100 ? "text-emerald-500" : ""}>
                      {latestConnected}
                    </span>
                  </div>
                  <div className="w-full h-3 bg-muted/30 rounded-full overflow-hidden border border-border/50">
                ...
                <div>
                  <div className="flex justify-between text-xs font-medium mb-1">
                    ...
                    <span className={batTalk >= 100 ? "text-emerald-500" : ""}>
                      {latestTalkTime}m
                    </span>
                  </div>
                  <div className="w-full h-3 bg-muted/30 rounded-full overflow-hidden border border-border/50">
            ...
            <div className="bg-card border border-border/60 rounded-2xl p-6 shadow-xs relative overflow-hidden lg:col-span-1 min-h-[220px]">
              ...
              <h3 className="font-semibold text-sm text-muted-foreground mb-4 uppercase tracking-wider text-center">
                Conversion Funnel
              </h3>
              ...
              <div className="flex flex-col items-center justify-center h-full space-y-1.5 w-full max-w-[200px] mx-auto pb-4">
                <div className="bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 w-full rounded-md py-1.5 text-center text-xs font-bold border border-indigo-500/30">
                  Dialed:{" "}
                  {userTimeSeries
                    .reduce((s, i) => s + (i.dialedCalls || 0), 0)
                    .toLocaleString()}
                </div>
                <div className="bg-blue-500/20 text-blue-700 dark:text-blue-300 w-4/5 rounded-md py-1.5 text-center text-xs font-bold border border-blue-500/30">
                  Connected:{" "}
                  {userTimeSeries
                    .reduce((s, i) => s + (i.connectedCalls || 0), 0)
                    .toLocaleString()}
                </div>
                ...
                <div className="bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 w-2/5 rounded-md py-1.5 text-center text-xs font-bold border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.3)]">
                  Sessions:{" "}
                  {userTimeSeries
                    .reduce((s, i) => s + (i.sessionBooked || 0), 0)
                    .toLocaleString()}
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/60 rounded-2xl p-6 shadow-xs relative overflow-hidden min-h-[220px] flex flex-col items-center justify-center">
              ...
              <div
                className={`relative flex items-center justify-center transition-all duration-700 ${currentStreak > 0 ? "scale-110" : "grayscale opacity-50"}`}
              >
              ...
              <div className="mt-8 text-center px-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Hit dial, connect & talk time targets to{" "}
                  {currentStreak > 0 ? "keep it alive." : "reignite the fire."}
                </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gradient-to-br from-blue-500/10 to-transparent border border-blue-200/50 dark:border-blue-900/30 rounded-2xl p-5 shadow-xs">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">
                Total {selectedUserMetric.replace(/([A-Z])/g, " $1")}
              </h3>
            </div>
          ...
          <div className="bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-200/50 dark:border-emerald-900/30 rounded-2xl p-5 shadow-xs">
            ...
            <p className="text-4xl font-bold text-foreground">{dailyAverage}</p>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold tracking-tight flex items-center gap-2">
              ...
              {selectedUser
                ? `${selectedUser}'s Analytics`
                : "EOD Analytics Dashboard"}
            </h2>
          </div>
          <p className="text-sm text-muted-foreground mt-1">
            {selectedUser
              ? `Detailed performance metrics for ${selectedUser}.`
              : "Track team output and employee EOD submissions."}
          </p>
        </div>
