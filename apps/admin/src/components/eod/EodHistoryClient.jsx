...
export function EodHistoryClient({
  ...
  const executeTriggerEmails = async (targetDate) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(
          `Sent ${res.reportsSent} team reports and ${res.warningsSent} warnings. ${res.failed?.length ? `(${res.failed.length} failed)` : ""}`,
        );
        fetchHistory();
      } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-border/50">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Review past submissions and trigger reminder emails.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-muted-foreground border border-dashed rounded-lg bg-slate-50/50 dark:bg-slate-900/20">
          ...
          <p className="text-base font-medium">
            No reports or pending submissions found.
          </p>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                      {missingUsers.map((u) => (
                        <div
                          key={u.id}
                          className="group flex items-center justify-between bg-background border border-border/60 hover:border-border rounded-md px-4 py-3 shadow-sm transition-colors"
                        >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    {dateReports.map((report) => (
                      <div
                        key={report.id}
                        className="flex flex-col bg-background border border-border/60 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all group"
                      >
                        <div
                            ...
                            <div className="flex items-center gap-3">
                              <div className="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-slate-300 font-medium text-sm border border-border/50 overflow-hidden shrink-0">
                                {report.profileImageKey ? (
                                  <img
                                    src={`/files/${report.profileImageKey}`}
                                    alt={report.userName}
                                    className="h-full w-full object-cover"
                                  />
                                ) : (
              ...
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-700 dark:text-slate-300 font-medium text-sm border border-border/50 overflow-hidden shrink-0">
                  {selectedReport.profileImageKey ? (
                    <img
                      src={`/files/${selectedReport.profileImageKey}`}
                ...
                <div>
                  ...
                  <div className="text-sm text-muted-foreground mt-0.5 flex items-center gap-2">
                    <span>
                      {TEAM_LABELS[selectedReport.team] || selectedReport.team}
                    </span>
                    <span>•</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                {Object.entries(selectedReport.data || {})
                  ...
                  .map(([k, v]) => (
                    <div
                      key={k}
                      className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-border/50 flex flex-col"
                    >
