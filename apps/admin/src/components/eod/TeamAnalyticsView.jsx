...
const TEAM_METADATA = {
  sales: {
    ...
    icon: PhoneCall,
    color:
      "from-blue-500/10 to-indigo-500/5 border-blue-500/30 text-blue-600 dark:text-blue-400",
  ...
  tech: {
    ...
    icon: Code,
    color:
      "from-emerald-500/10 to-teal-500/5 border-emerald-500/30 text-emerald-600 dark:text-emerald-400",
    badge:
  ...
  marketing: {
    ...
    icon: Megaphone,
    color:
      "from-purple-500/10 to-pink-500/5 border-purple-500/30 text-purple-600 dark:text-purple-400",
    badge:
  ...
  hr: {
    ...
    icon: UserCheck,
    color:
      "from-amber-500/10 to-orange-500/5 border-amber-500/30 text-amber-600 dark:text-amber-400",
    badge:
  ...
  ceo_office: {
    ...
    icon: Award,
    color:
      "from-rose-500/10 to-red-500/5 border-rose-500/30 text-rose-600 dark:text-rose-400",
  ...
  admin_head: {
    ...
    icon: Layers,
    color:
      "from-sky-500/10 to-cyan-500/5 border-sky-500/30 text-sky-600 dark:text-sky-400",
  ...
  outside_sales: {
    ...
    icon: Briefcase,
    color:
      "from-indigo-500/10 to-violet-500/5 border-indigo-500/30 text-indigo-600 dark:text-indigo-400",
    badge:
...
export function TeamAnalyticsView({
  ...
  const processedTeams = useMemo(() => {
    return teamAggregates.map((teamObj) => {
      ...
      const meta = TEAM_METADATA[teamKey] || {
        ...
        icon: Briefcase,
        color:
          "from-slate-500/10 to-gray-500/5 border-slate-500/30 text-slate-600",
      ...
      teamReports.forEach((r) => {
        ...
        Object.entries(r.data || {}).forEach(([k, v]) => {
          ...
          if (!isNaN(num) && typeof v !== "boolean" && k !== "notes") {
            membersMap[r.userName].metrics[k] =
              (membersMap[r.userName].metrics[k] || 0) + num;
          }
        });
      });

      const members = Object.values(membersMap).sort(
      ...
      const connectRate =
        dialed > 0 ? ((connected / dialed) * 100).toFixed(1) : "0.0";
  ...
  const pipelineData = useMemo(() => {
    ...
    const timeSeries = Object.values(ts).sort((a, b) =>
      a.date.localeCompare(b.date),
    );
    const totalBooked = salesReports.reduce(
      (s, r) => s + Number(r.data?.counsellingBooked || 0),
      0,
    );
    const totalDone = salesReports.reduce(
      (s, r) => s + Number(r.data?.counsellingDone || 0),
      0,
    );
    const totalWalkin = salesReports.reduce(
      (s, r) => s + Number(r.data?.walkinCounselling || 0),
      0,
    );
    const totalConducted = totalDone + totalWalkin;
    const totalSessions = salesReports.reduce(
      (s, r) => s + Number(r.data?.sessionBooked || 0),
      0,
    );
    const bookedToConducted =
      totalBooked > 0 ? Math.round((totalConducted / totalBooked) * 100) : 0;
    const bookedToSessions =
      totalBooked > 0 ? Math.round((totalSessions / totalBooked) * 100) : 0;
    return {
      timeSeries,
      totalBooked,
      totalDone,
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-card border border-border/60 rounded-2xl p-4 shadow-xs flex items-center gap-3.5">
          ...
          <div className="overflow-hidden">
            <div className="text-[11px] sm:text-xs text-muted-foreground font-semibold uppercase tracking-wider truncate">
              Active Employees
            </div>
            <div className="text-xl sm:text-2xl font-extrabold text-foreground">
              {overallTotals.totalMembers}
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl p-4 shadow-xs flex items-center gap-3.5">
          ...
          <div className="overflow-hidden">
            ...
            <div className="text-xl sm:text-2xl font-extrabold text-foreground">
              {overallTotals.totalSalesCalls.toLocaleString()}
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-border/60 rounded-2xl p-4 shadow-xs flex items-center gap-3.5">
          ...
          <div className="overflow-hidden">
            ...
            <div className="text-xl sm:text-2xl font-extrabold text-foreground">
              {overallTotals.totalTalkTimeHours} hrs
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {displayedTeams.map((t) => {
            ...
            return (
              <div
                key={t.teamKey}
                className="bg-card border border-border/60 rounded-3xl p-4 sm:p-5 shadow-xs space-y-4"
              >
                <div className="flex items-center justify-between gap-3 border-b border-border/40 pb-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2.5 rounded-2xl bg-gradient-to-br ${t.meta.color} border border-border/40`}
                    >
                    ...
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {t.memberCount} Team Members
                      </div>
                    </div>
                  </div>
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-bold border ${t.meta.badge}`}
                  >
                ...
                <div className="space-y-2 pt-1">
                  ...
                  {t.members.length === 0 ? (
                    <p className="text-xs text-muted-foreground italic">
                      No submissions for selected date range.
                    </p>
                  ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 text-center">
              ...
              <div className="text-2xl font-extrabold text-amber-600 dark:text-amber-400">
                {pipelineData.totalBooked.toLocaleString()}
              </div>
            ...
            <div className="bg-purple-500/5 border border-purple-500/20 rounded-2xl p-4 text-center relative">
              ...
              <div className="text-2xl font-extrabold text-purple-600 dark:text-purple-400">
                {pipelineData.totalConducted.toLocaleString()}
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                {pipelineData.totalDone} done · {pipelineData.totalWalkin}{" "}
                walk-in
              </div>
            </div>
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-4 text-center relative">
              ...
              <div className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400">
                {pipelineData.totalSessions.toLocaleString()}
              </div>
              <div className="text-xs text-muted-foreground mt-1">
                final conversions
              </div>
