  ...
  if (schema.type === "object") {
    ...
    if (schema.required) {
      for (const req of schema.required) {
        if (!(req in obj)) {
          throw new Error(
            `Validation Error: Required field "${req}" is missing in ${path}.`,
          );
        }
      }
    }
    ...
    if (typeof obj !== "string" && (!schema.nullable || obj !== null)) {
      throw new Error(
        `Validation Error: ${path} should be a string (or null if allowed).`,
      );
    }
    if (schema.enum && !schema.enum.includes(obj)) {
      throw new Error(
        `Validation Error: ${path} value "${obj}" is not in allowed enum [${schema.enum.join(", ")}].`,
      );
    }
...
const mockAnalysisResult = {
  ...
  scriptAdherence: {
    authorityIntro: {
      status: "completed",
      evidence: "Introduced as counselor",
    },
    ...
    situationDiscovery: {
      status: "completed",
      evidence: "Asked for background info",
    },
    problemGapIdentified: {
      status: "completed",
      evidence: "Highlighted lack of skills",
    },
    implicationAmplified: {
      status: "completed",
      evidence: "Discussed job security",
    },
    qualificationQuestions: {
      status: "completed",
      evidence: "Validated career goal",
    },
    decisionMakerIdentified: {
      status: "completed",
      evidence: "Asked about parents",
    },
    valueStackPitch: {
      status: "completed",
      evidence: "Pitched curriculum value",
    },
    programModelExplained: {
      status: "completed",
      evidence: "Explained 2+1 campus plan",
    },
    objectionHandling: {
      status: "completed",
      evidence: "Addressed fees skepticism",
    },
...
try {
  ...
  console.log(
    "\x1b[32m%s\x1b[0m",
    "SUCCESS: Schema conformance validated successfully! Mock data matches RESPONSE_SCHEMA structure.",
  );
} catch (err) {
  console.error(
    "\x1b[31m%s\x1b[0m",
    "FAILURE: Mock data fails schema validation!",
  );
